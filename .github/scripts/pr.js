...
export async function processPrValidation({ github, context, core }) {
  ...
  if (pr.draft) {
    lines.push(
      checklistLine("Draft PR mode", true, "full validation deferred until ready for review"),
    );
  } else {
...
export async function processFirstContributorWelcome({ github, context, core }) {
  ...
  if ((existing || []).some((comment) => hasMarker(comment.body, AUTOMATION.firstWelcomeMarker)))
    return;
