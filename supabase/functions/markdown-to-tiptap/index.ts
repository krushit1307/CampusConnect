...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!file.name.endsWith('.md') && file.type !== 'text/markdown') {
      return new Response(
        JSON.stringify({ error: 'File must be a Markdown (.md) file' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    return new Response(
      JSON.stringify({ error: 'Internal server error during parsing' }),
...
function mapHtmlNodeToTiptap(node: any): any {
  ...
  if (node.nodeType === 1) { // Element node
    ...
    const flatText = children.filter(c => c.type === 'text').map(c => c.text).join(' ');

    switch (tagName) {
      case 'h1':
        return { type: 'heading', attrs: { level: 1 }, content: [{ type: 'text', text: node.textContent.trim() }] };
      case 'h2':
        return { type: 'heading', attrs: { level: 2 }, content: [{ type: 'text', text: node.textContent.trim() }] };
      case 'h3':
        return { type: 'heading', attrs: { level: 3 }, content: [{ type: 'text', text: node.textContent.trim() }] };
      case 'p':
      ...
      case 'ul':
        return { 
          ...
          content: Array.from(node.children).map((li: any) => ({
            type: 'listItem',
            content: [{ type: 'paragraph', content: [{ type: 'text', text: li.textContent.trim() }] }]
          }))
        };
      case 'ol':
        return { 
      ...
      case 'blockquote':
        return { type: 'blockquote', content: [{ type: 'paragraph', content: [{ type: 'text', text: node.textContent.trim() }] }] };
      case 'code':
