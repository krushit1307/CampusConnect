...
function parseDERSignature(der: Uint8Array): { r: Uint8Array; s: Uint8Array } {
  ...
  function readInteger(): Uint8Array {
    const tag = readByte();
    if (tag !== 0x02)
      throw new Error(`DER: expected INTEGER tag (0x02), got 0x${tag.toString(16)}`);
  ...
  if (seqTag !== 0x30)
    throw new Error(`DER: expected SEQUENCE tag (0x30), got 0x${seqTag.toString(16)}`);
...
function encodeIntegerDER(value: Uint8Array): Uint8Array {
  ...
  const valueBytes = needsPad ? new Uint8Array([0x00, ...trimmed]) : trimmed;
...
export function parseAuthenticatorData(authData: Uint8Array) {
  ...
  const signCount =
    ((authData[33] << 24) | (authData[34] << 16) | (authData[35] << 8) | authData[36]) >>> 0; // >>> 0 coerces to unsigned 32-bit integer
...
export async function verifyRpIdHash(rpIdHash: Uint8Array, expectedRpId: string): Promise<boolean> {
  ...
  const expectedHash = new Uint8Array(await crypto.subtle.digest("SHA-256", rpIdBytes));
