    ...
    try {
        ...
        if (email) {
            ...
            const { data: lookupRows, error: lookupError } = await supabase
                .rpc("get_user_id_by_email", { target_email: email.toLowerCase().trim() });
        ...
        if (challengeError) {
            ...
            return new Response(
                JSON.stringify({ error: "Failed to generate authentication options", details: challengeError.message }),
                {
                    status: 500,
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
                },
