...
async function buildAuthenticatorData(opts: {
    ...
    let pos = 0;
    full.set(base, pos); pos += 37;
    full.set(aaguid, pos); pos += 16;
    full.set(credIdLen, pos); pos += 2;
    full.set(opts.credentialId, pos); pos += opts.credentialId.length;
...
function p1363ToDER(p1363: Uint8Array): Uint8Array {
    ...
    let pos = 0;
    der[pos++] = 0x30; der[pos++] = seqLen;
    der[pos++] = 0x02; der[pos++] = rDer.length;
    der.set(rDer, pos); pos += rDer.length;
    der[pos++] = 0x02; der[pos++] = sDer.length;
    der.set(sDer, pos);
    return der;
}
...
Deno.test("verifySignature: ES256 — wrong signature returns false", async () => {
    ...
    badSignature[0] = 0x30; badSignature[1] = 70;
    badSignature[2] = 0x02; badSignature[3] = 32;
    badSignature[36] = 0x02; badSignature[37] = 32;
...
Deno.test("verifySignature: ES256 — tampered authData returns false", async () => {
    ...
    const result = await verifySignature(cosePublicKeyBytes, signature, clientDataHash, tamperedAuthData);
    assertEquals(result, false);
});

Deno.test("verifySignature: ES256 — userPresent=false in authData does not affect crypto (checked at app layer)", async () => {
    ...
    const result = await verifySignature(cosePublicKeyBytes, signature, clientDataHash, authData);
    assertEquals(result, true, "crypto layer should not enforce UP flag — that is the caller's responsibility");
});
...
Deno.test("buildAuthenticatorData: flags are set correctly", async () => {
    const authDataUP = await buildAuthenticatorData({ rpId: "x", userPresent: true, userVerified: false });
    assertEquals(authDataUP[32] & 0x01, 1); // UP set
    assertEquals(authDataUP[32] & 0x04, 0); // UV not set

    const authDataBoth = await buildAuthenticatorData({ rpId: "x", userPresent: true, userVerified: true });
...
Deno.test("Challenge security: expired challenges are not returned by DB query (expiry in WHERE clause)", () => {
    ...
    assertEquals(filteredOut, true, "Expired challenge should be filtered out by DB gt(expires_at, now)");
});

Deno.test("Challenge security: replay — second use of same challenge ID returns null (simulated deletion)", () => {
