 ...
 */
export const replicaClient =
    global.__replicaPrisma ??
    new PrismaClient(replicaPrismaConfig);
...
replicaClient.$use(async (params, next) => {
    const mutatingActions = ['create', 'update', 'delete', 'upsert', 'createMany', 'updateMany', 'deleteMany'];
...
export async function runAnalyticsQuery<T>(
    ...
    return replicaClient.$transaction(
