...
import { GraduationCap, Users, ClipboardList, CheckCircle2, AlertTriangle, ArrowRight, Mail, CalendarDays, Briefcase, Sparkles } from 'lucide-react';
...
export default function SeniorOffboarding() {
  ...
  const updateHandoverStatus = (id: string) => {
    setSeniors(prev => prev.map(senior => {
      if (senior.id === id) {
        const nextStatus = 
          senior.handoverStatus === 'Pending' ? 'In Progress' :
          senior.handoverStatus === 'In Progress' ? 'Completed' : 'Pending';
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-slate-400 text-xs flex items-center gap-1">
                  <Users className="w-3.5 h-3.5 text-orange-400" /> {seniors.length} Graduating Seniors
                </span>
              </div>
            ...
            <div className="flex items-center gap-3">
              <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4">
                <p className="text-2xl font-bold text-emerald-400">{completedCount}/{seniors.length}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-slate-900/80 border border-slate-800 rounded-3xl overflow-hidden">
            <div className="p-6 border-b border-slate-800">
              <h2 className="text-xl font-bold flex items-center gap-2"><Users className="w-5 h-5 text-amber-400" /> Senior Leadership</h2>
            </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                        <span className="flex items-center gap-1"><CalendarDays className="w-3 h-3" /> {senior.graduationDate}</span>
                        <span className="flex items-center gap-1"><Mail className="w-3 h-3" /> {senior.email}</span>
                      </div>
                    </div>
                  </div>
                  <button 
                    onClick={(e) => { e.stopPropagation(); updateHandoverStatus(senior.id); }}
                    className={`px-3 py-1.5 rounded-full text-xs font-bold transition flex items-center gap-1.5 ${
                      senior.handoverStatus === 'Completed' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' :
                      senior.handoverStatus === 'In Progress' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/30' :
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6">
            ...
            {activeSenior ? (
              <div className="space-y-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Offboarding Tasks</p>
                ...
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <p className="text-xs text-amber-300 leading-relaxed">
                    This is a standalone dashboard component. It does not modify any existing backend data or exports.
                  </p>
                </div>
