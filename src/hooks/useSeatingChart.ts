...
interface UseSeatingChartReturn {
    ...
    assignSeat: (chairId: string, tableId: string, userId: string | null, userName: string | null) => void;
...
export function useSeatingChart(eventId: string): UseSeatingChartReturn {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (chartError && chartError.code !== 'PGRST116') { // PGRST116 = No rows found
                    throw chartError;
                }
    ...
    const addTable = useCallback((type: 'round' | 'rectangle') => {
    ...
    const assignSeat = useCallback((chairId: string, tableId: string, userId: string | null, userName: string | null) => {
        setCanvasState(prev => ({
            ...prev,
            tables: prev.tables.map(t => {
                if (t.id === tableId) {
                ...
                if (userId) {
                    return {
                        ...t,
                        chairs: t.chairs.map(c =>
                            c.assignedUserId === userId ? { ...c, assignedUserId: null, assignedUserName: null } : c
                        )
                    };
                }
                return t;
