    ...
    const fetchPasskeys = useCallback(async () => {
        ...
        try {
            const { data, error: fetchErr } = await supabase
                ...
                .select(
                    "id, credential_id, device_name, transports, created_at, last_used_at, backed_up",
                )
        ...
        async (deviceName?: string): Promise<boolean> => {
            ...
            try {
                ...
                const verifyRes = await fetch(
                    `${supabaseUrl}/functions/v1/webauthn-registration-verify`,
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
    ...
    const authenticateWithPasskey = useCallback(
        async (
            ...
            try {
                ...
                const optionsRes = await fetch(
                    `${supabaseUrl}/functions/v1/webauthn-auth-options`,
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                ...
                const verifyRes = await fetch(
                    `${supabaseUrl}/functions/v1/webauthn-auth-verify`,
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
    ...
    const renamePasskey = useCallback(
        async (id: string, newName: string): Promise<boolean> => {
            ...
            try {
                ...
                setPasskeys((prev) =>
                    prev.map((p) => (p.id === id ? { ...p, device_name: newName } : p)),
                );
                return true;
            } catch (err) {
                const message =
