...
class MockBroadcastChannel {
    ...
    postMessage(data: unknown) {
        ...
        const others = (MockBroadcastChannel.instances[this.name] ?? []).filter(
            (ch) => ch !== this
        );
...
describe("useBroadcastState", () => {
    ...
    it("degrades gracefully when BroadcastChannel is unavailable", () => {
        ...
        const original = (globalThis as { BroadcastChannel?: typeof MockBroadcastChannel }).BroadcastChannel;
