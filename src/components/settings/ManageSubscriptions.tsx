            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
                {subscriptions.length === 0 ? (
                    <div className="p-12 text-center text-gray-500 dark:text-gray-400">
                        ...
                        <p className="text-sm mt-1">Visit a club's profile and click the bell icon to subscribe to their events.</p>
                    </div>
                ) : (
                    subscriptions.map(sub => (
                        <div key={sub.id} className="p-4 flex items-center gap-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            <div className="w-12 h-12 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center overflow-hidden flex-shrink-0">
                                {sub.clubs.logo_url ? (
                                    <img src={sub.clubs.logo_url} alt={sub.clubs.name} className="w-full h-full object-cover" />
                                ) : (
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                    Events: {sub.notify_events ? '✅' : '❌'} • Announcements: {sub.notify_announcements ? '✅' : '❌'}
                                </p>
                            </div>
