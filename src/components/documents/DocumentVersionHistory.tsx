...
export const DocumentVersionHistory: React.FC<DocumentVersionHistoryProps> = ({ documentId }) => {
    ...
    const [selectedVersions, setSelectedVersions] = useState<{ old: DocumentVersion | null; new: DocumentVersion | null }>({
        old: null,
        new: null
    });
            ...
            <div className="w-80 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 flex flex-col h-full overflow-hidden">
                <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Current: <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400">v{document?.current_version}</span>
                    </p>
                </div>
                        ...
                        <div className="space-y-6">
                            {versions.map((version, index) => (
                                <div key={version.id} className="relative pl-10">
                                    {/* Timeline Node */}
                                    <div className={`absolute left-0 w-8 h-8 rounded-full flex items-center justify-center ring-4 ring-white dark:ring-gray-800 ${index === 0
                                        ...
                                        {index === 0 ? (
                                            ...
                                            <span className="text-xs font-bold">{version.version_number.split('.')[0]}</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${version.version_type === 'major' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' :
                                                    version.version_type === 'minor' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' :
                                        ...
                                        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-500 mb-3">
                                            ...
                                            <span className="truncate ml-2">{version.uploader_profile?.full_name || 'Unknown'}</span>
                                        </div>
