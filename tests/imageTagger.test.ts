...
beforeEach(() => {
    ...
    (globalThis as unknown as { HTMLCanvasElement: typeof MockCanvas }).HTMLCanvasElement = MockCanvas as unknown as typeof HTMLCanvasElement;
...
describe("imageTagger — autoTagImage", () => {
    ...
    it("returns error when the file is not an image", async () => {
        const result = await autoTagImage(new File([new Uint8Array(100)], "doc.pdf", { type: "application/pdf" }));
        expect(result.ok).toBe(false);
        if (!result.ok) {
